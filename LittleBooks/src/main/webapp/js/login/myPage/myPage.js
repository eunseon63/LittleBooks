...
$(function(){
	...
	$("span#emailcheck").click(function(){
		 ...
		 $.ajax({
			 url:"/LittleBooks/register/emailDuplicateCheck2.go",
...
//  또는
	...
	if(isNewPwd) { // 새암호가 기존암호와 다른 경우
	//	alert("DB에 사용자 정보를 수정하러 간다.");
		
		const frm = document.myPageUpdate;
		frm.action = "/LittleBooks/login/myPageUpdate.go";
		frm.method = "post";
		frm.submit();	
	}
